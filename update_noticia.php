...
function fetch_noticia_row(PDO $pdo, int $noticiaId, bool $forUpdate = false): ?array {
  $sql = "
    SELECT
      n.id,
      n.noticia,
      ...
      CONCAT_WS(' ', cc.nombre, cc.apellidos) AS cliente,
      cc.telefono AS cliente_whatsapp,
      COALESCE(NULLIF(cc.email, ''), uc.email) AS cliente_correo,
      n.domicilio,
      n.ubicacion_en_mapa,
      n.reportero_id,
...
$titulo = array_key_exists('noticia', $_POST)
  ? trim((string)$_POST['noticia'])
  : null;
...
if ($tipoDeNota !== null) {
  if ($tipoDeNota === '') {
    ...
    if ($tipoDeNota === 'Noticia') {
      $tipoDeNota = 'Noticia';
    }

    $allowed = ['Noticia', 'Entrevista', 'Reportaje'];
...
// Compatibilidad: acepta cliente_cliente_id o cliente_id viejo
$hasClienteClienteId =
  array_key_exists('cliente_cliente_id', $_POST) ||
  array_key_exists('cliente_id', $_POST);

$clienteClienteIdReq = null;
if (array_key_exists('cliente_cliente_id', $_POST)) {
  $clienteClienteIdReq = trim((string)$_POST['cliente_cliente_id']);
} elseif (array_key_exists('cliente_id', $_POST)) {
  $clienteClienteIdReq = trim((string)$_POST['cliente_id']);
}

$clienteClienteIdParsed = null;
if ($hasClienteClienteId) {
  if ($clienteClienteIdReq === '' || $clienteClienteIdReq === '0') {
    $clienteClienteIdParsed = null;
  } else {
    $tmp = (int)$clienteClienteIdReq;
    $clienteClienteIdParsed = ($tmp > 0) ? $tmp : null;
  }
}

$hasUsuarioClienteId = array_key_exists('usuario_cliente_id', $_POST);
$usuarioClienteIdReq = $hasUsuarioClienteId
  ? trim((string)$_POST['usuario_cliente_id'])
  : null;

$usuarioClienteIdParsed = null;
if ($hasUsuarioClienteId) {
  if ($usuarioClienteIdReq === '' || $usuarioClienteIdReq === '0') {
    $usuarioClienteIdParsed = null;
  } else {
    $tmp = (int)$usuarioClienteIdReq;
    $usuarioClienteIdParsed = ($tmp > 0) ? $tmp : null;
  }
}
...
if (($hasClienteClienteId || $hasUsuarioClienteId || $hasDomicilio) && $role !== 'admin') {
...
try {
  ...
  $actual = fetch_noticia_row($pdo, $noticiaId, true);
  ...
  // Resolver cliente/usuario del esquema nuevo
  if ($hasClienteClienteId && $clienteClienteIdParsed !== null) {
    $stmtCli = $pdo->prepare("
      SELECT usuario_id
      ...
      LIMIT 1
    ");
    $stmtCli->execute([$clienteClienteIdParsed]);
    $resolvedUsuario = $stmtCli->fetchColumn();

    if (!$resolvedUsuario) {
      $pdo->rollBack();
      echo json_encode(['success' => false, 'message' => 'Cliente seleccionado no encontrado']);
      exit;
    }

    $usuarioClienteIdParsed = (int)$resolvedUsuario;
  } elseif (!$hasClienteClienteId && $hasUsuarioClienteId && $usuarioClienteIdParsed !== null) {
    $stmtCli = $pdo->prepare("
      SELECT id
      FROM clientes_clientes
      WHERE usuario_id = ?
      LIMIT 1
    ");
    $stmtCli->execute([$usuarioClienteIdParsed]);
    $resolvedCliente = $stmtCli->fetchColumn();

    if (!$resolvedCliente) {
      $pdo->rollBack();
      echo json_encode(['success' => false, 'message' => 'usuario_cliente_id no tiene cliente asociado']);
      exit;
    }

    $clienteClienteIdParsed = (int)$resolvedCliente;
    $hasClienteClienteId = true;
  }
  ...
  $oldClienteClienteId = !empty($actual['cliente_cliente_id'])
    ? (int)$actual['cliente_cliente_id']
    : 0;

  $oldUsuarioClienteId = !empty($actual['usuario_cliente_id'])
    ? (int)$actual['usuario_cliente_id']
    : 0;
  ...
  if ($hasClienteClienteId) {
    $clienteAsignadoAhora =
      ($oldClienteClienteId <= 0) &&
      ($clienteClienteIdParsed !== null) &&
      ($clienteClienteIdParsed > 0);
  }
  ...
  // ========================= ADMIN =========================
  if ($role === 'admin') {
    if ($hasClienteClienteId || $hasUsuarioClienteId) {
      if ($clienteClienteIdParsed === null) {
        $updates[] = "cliente_cliente_id = NULL";
        $updates[] = "usuario_cliente_id = NULL";
        $updates[] = "domicilio = NULL";
      } else {
        if ($clienteClienteIdParsed !== $oldClienteClienteId) {
          $updates[] = "cliente_cliente_id = :cliente_cliente_id";
          $params[':cliente_cliente_id'] = $clienteClienteIdParsed;
        }

        if ($usuarioClienteIdParsed !== null && $usuarioClienteIdParsed !== $oldUsuarioClienteId) {
          $updates[] = "usuario_cliente_id = :usuario_cliente_id";
          $params[':usuario_cliente_id'] = $usuarioClienteIdParsed;
        }
      }
    }

    if ($hasDomicilio && !(($hasClienteClienteId || $hasUsuarioClienteId) && $clienteClienteIdParsed === null)) {
    ...
    if ($hasFechaCita && $changedFechaReal) {
  ...
  if ($startedRouteNow) {
    $mailType = 'route_started';
  } elseif ($clienteAsignadoAhora) {
    $mailType = 'cliente_assigned';
  } elseif ($changedFechaReal) {
    $mailType = 'rescheduled';
  } elseif ($changedDomReal) {
    $mailType = 'domicilio_changed';
  }

  try {
    $correoCliente = trim((string)($row['cliente_correo'] ?? ''));
    ...
    if ($mailType === null) {
      ...
      if ($mailType === 'route_started') {
        ...
        if ($reporteroTxt !== '') $details[] = ['Reportero', $reporteroTxt];
        $details[] = ['Estatus', 'Actualización de domicilio'];
